...
object NewsRepository {
    ...
    suspend fun fetch(context: Context): List<NewsItem> = withContext(Dispatchers.IO) {
        val fresh = feeds.filter { settings(context).getBoolean("source_${it.name}", true) }
            .flatMap { feed -> runCatching { parseFeed(feed) }.getOrDefault(emptyList()) }
            .filter { relevant(it) }
        ...
        fresh.ifEmpty { loadCachedNews(context) }
    }
    ...
    private fun parseFeed(feed: Feed): List<NewsItem> {
        ...
        return doc.select("item").mapNotNull { item ->
            ...
            val published = parseDate(item.selectFirst("pubDate")?.text()) ?: System.currentTimeMillis()
    ...
    private fun parseDate(raw: String?): Long? {
        ...
        val patterns = listOf("EEE, dd MMM yyyy HH:mm:ss z", "EEE, dd MMM yyyy HH:mm z", "yyyy-MM-dd'T'HH:mm:ssX")
    ...
    suspend fun translateToPersian(text: String): String = withContext(Dispatchers.IO) {
        ...
        runCatching {
            ...
            val translated = chunks.map { tr.translate(it).await() }.joinToString("\n")
            tr.close()
            translated
        }.getOrDefault(text)
    }
    ...
    suspend fun dollarRate(): String = withContext(Dispatchers.IO) {
        runCatching {
            ...
            if (last.isNullOrBlank()) "در دسترس نیست" else buildString {
                append(last.replace(",", "").toLongOrNull()?.let { String.format(Locale.US, "%,d", it) } ?: last)
                append(" تومان")
                if (!yesterday.isNullOrBlank()) append("\nدیروز: $yesterday")
                if (!change.isNullOrBlank()) append(" • تغییر: $change")
            }
