...
class NewsWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    override suspend fun doWork(): Result = runCatching {
        ...
        news.filter { it.importance >= 55 && !seen.contains(it.id) }
            ...
            .forEach { item ->
                val pending = PendingIntent.getActivity(
                    applicationContext,
                    item.id.hashCode(),
                    Intent(applicationContext, com.nabz.news.MainActivity::class.java).apply {
                ...
                nm.notify(
                    ...
                    NotificationCompat.Builder(applicationContext, "news")
                        .setSmallIcon(R.drawable.ic_nabz)
                        .setContentIntent(pending)
                        .setContentTitle("نبض • $label • ${item.source}")
        ...
        prefs.edit()
            .putStringSet("seen_ids", seen.toList().takeLast(300).toSet())
            .apply()
